...
<style>
    ...
    #itemGrid .item.pointer-events-none {
        cursor: not-allowed;
    }
</style>
...
<div class="flex flex-col h-full md:flex-row">
    ...
    <div class="bg-white p-4 rounded shadow w-full md:w-1/3 mt-2 md:mt-0">
        ...
        <div class="space-y-2 mb-4">
            ...
            <div class="flex justify-between items-center">
                <label for="serviceType" class="font-semibold">Service Type:</label>
                <select id="serviceType" class="border p-1 w-1/2">
                    ...
                    <optgroup label="Laboratory">
                        <option value="Digital x-ray w/ fluoroscopy">Digital x-ray w/ fluoroscopy</option>
                        <option value="Gas anaesthesia">Gas anaesthesia</option>
                        <option value="Complete blood count">Complete blood count</option>
                        <option value="Ultrasound">Ultrasound</option>
                        <option value="Microscopy">Microscopy</option>
                        <option value="Test kits">Test kits</option>
                        <option value="Smear test">Smear test</option>
...
<form id="posForm" method="POST" action="{{ route('pos.store') }}" class="hidden">
    ...
    <input type="hidden" name="service" id="formServiceType">
...
<div id="stockModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
    <div class="bg-white p-6 rounded shadow-lg w-[90%] max-w-sm">
        <h2 class="text-lg font-bold text-red-600 mb-4">Stock Limit Reached</h2>
        <p id="stockMessage" class="text-gray-700 mb-6"></p>
        <div class="flex justify-end">
            <button onclick="closeStockModal()" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
                OK
            </button>
        </div>
    </div>
...
<script>
    ...
    function addToCart(parent) {
        ...
        if (cartItem.qty < stock) {
            ...
            showStockModal(`Only ${stock} stocks available for ${cartItem.name}.`);
        }
    }
    ...
    function updateQty(index, value) {
        ...
        if (qty > stock) {
            qty = stock;
            showStockModal(`Maximum stock for ${cart[index].name} is ${stock}.`);
        }
        cart[index].qty = qty;
        renderCart();
    }
    ...
    function showConfirmationModal() {
        ...
        const discount = totalBeforeDiscount * (discPercent / 100);
        const total = totalBeforeDiscount - discount;
        const serviceType = document.getElementById('serviceType').value || 'N/A';
        receipt += `<p>Service Type: ${serviceType}</p>
    ...
    function submitCart() {
        ...
        document.getElementById('formServiceType').value = document.getElementById('serviceType').value || '';
    ...
    function printReceipt() {
        ...
        <head>
            ...
            <style>
                ...
                .logo { width: 60px; height: 60px; background-color: #f0f0f0; border-radius: 50%; margin: 0 auto 15px;
                        display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 24px; color: #666; }
        ...
        <body>
            ...
            <div class="customer-info">
                ...
                <div class="customer-name">Service: ${serviceType}</div>
            </div>
            <div class="items-summary">${cart.length} items (Qty.: ${cart.reduce((t, i) => t + i.qty, 0)})</div>
